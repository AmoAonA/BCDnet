...
def train_one_epoch_(cfg, model, optimizer, data_loader, device, epoch, scaler, tfboard, softmax_criterion_s2,
                     softmax_criterion_s3, softmax_criterion_s4, outsys_dir=None):
    model.train()
    ...
    if epoch == 0:
        warmup_factor = cfg.SOLVER.WARMUP_EPOCHS
    ...
    for i, (images, targets) in enumerate(
        ...
        with amp.autocast():
            loss_dict, feats_reid_2nd, targets_reid_2nd, feats_reid_3rd, \
            targets_reid_3rd, feats_reid_4th, targets_reid_4th = model(images, targets)
            if cfg.MODEL.LOSS.USE_SOFTMAX:
                softmax_loss_2nd = cfg.SOLVER.LW_RCNN_SOFTMAX_2ND * softmax_criterion_s2(feats_reid_2nd,
                                                                                         targets_reid_2nd)
                softmax_loss_3rd = cfg.SOLVER.LW_RCNN_SOFTMAX_3RD * softmax_criterion_s3(feats_reid_3rd,
                                                                                         targets_reid_3rd)
                softmax_loss_4th = cfg.SOLVER.LW_RCNN_SOFTMAX_4TH * softmax_criterion_s4(feats_reid_4th,
                                                                                         targets_reid_4th)

                loss_dict.update(loss_box_softmax_2nd=softmax_loss_2nd)
                loss_dict.update(loss_box_softmax_3rd=softmax_loss_3rd)
                loss_dict.update(loss_box_softmax_4th=softmax_loss_4th)
            losses = sum(loss for loss in loss_dict.values() if 'corr' not in loss_dict.keys())  # model返回值的问题

        # reduce losses over all GPUs for logging purposes 聚合多GPU损失方便记录
        loss_dict_reduced = reduce_dict(loss_dict)
        losses_reduced = sum(loss for loss in loss_dict_reduced.values())
        loss_value = losses_reduced.item()

        if not math.isfinite(loss_value):  # 若loss_value是错误的，跳出
...
def train_one_epoch_1(cfg, model, optimizer, data_loader, device, epoch, scaler, tfboard, softmax_criterion_s2,
                     softmax_criterion_s3, softmax_criterion_s4, outsys_dir=None):
    model.train()
    metric_logger = MetricLogger(delimiter="  ", txt_dir=os.path.join(outsys_dir, 'os.txt'))
    metric_logger.add_meter("lr", SmoothedValue(window_size=1, fmt="{value:.6f}"))
    ...
    for i, (images, targets) in enumerate(
        ...
        with amp.autocast():
            ...
            if cfg.MODEL.LOSS.USE_SOFTMAX:
                ...
                loss_dict.update(loss_box_softmax_3rd=softmax_loss_3rd)
                loss_dict.update(loss_box_softmax_4th=softmax_loss_4th)
            losses = sum(loss for loss in loss_dict.values())  # model返回值的问题
...
def train_one_epoch(cfg, model, optimizer, data_loader, device, epoch, scaler, tfboard, softmax_criterion_s2,
                    softmax_criterion_s3, outsys_dir=None):
    model.train()
    metric_logger = MetricLogger(delimiter="  ", txt_dir=os.path.join(outsys_dir, 'os.txt'))
    metric_logger.add_meter("lr", SmoothedValue(window_size=1, fmt="{value:.6f}"))
    header = "Epoch: [{}]".format(epoch)
...
def evaluate_performance(
    ...
    if use_cache:
        eval_cache = torch.load(os.path.join(outsys_dir, "eval_cache/eval_cache.pth"))  # 加载保存点
