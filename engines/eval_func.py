...
def eval_detection(
    ...
    for anno, det in zip(annos, gallery_dets):  # 对每一张图 的 标签和检测结果进行对齐
        gt_boxes = anno["boxes"]
        if labeled_only:
        ...
        if det != []:
            ...
            num_det = 0
        if num_det == 0:  # 这图查询数是0，则累加真实框？未检测的框？
            count_gt += num_gt
            continue

        ious = np.zeros((num_gt, num_det), dtype=np.float32) # iou 检测框和gt联合计算
        for i in range(num_gt):
            for j in range(num_det):
                ious[i, j] = _compute_iou(gt_boxes[i], det[j, :4])
        tfmat = ious >= iou_thresh
        # for each det, keep only the largest iou of all the gt 对det找最大gt
        for j in range(num_det):
            largest_ind = np.argmax(ious[:, j])
            for i in range(num_gt):
                if i != largest_ind:
                    tfmat[i, j] = False
        # for each gt, keep only the largest iou of all the det 对gt找最大det
        for i in range(num_gt):
        ...
        for j in range(num_det):
            y_score.append(det[j, -1])  # 加入对应分数
            y_true.append(tfmat[:, j].any()) # 加入对应位置
        count_tp += tfmat.sum()
        count_gt += num_gt
...
def eval_search_cuhk(
    ...
    """
    gallery_dataset/query_dataset: an instance of BaseDataset (k) k为batch_size (q) q 为查询人数量
    gallery_det (list of ndarray): n_det x [x1, x2, y1, y2, score] per image (k,n_det,5)
    gallery_feat (list of ndarray): n_det x D features per image (k,n_det,D)
    query_feat (list of ndarray): D dimensional features per query image (q,D)
    query_box_feat(list of ndarray):query in image [label,x1,x2,y1,y2] 我猜的
    det_thresh (float): filter out gallery detections whose scores below this
