...
from loss.oim import LOIMLoss, OIMLoss
...
class COAT(nn.Module):
    def __init__(self, cfg):
        ...
        self.lw_rcnn_reid_4th = cfg.SOLVER.LW_RCNN_REID_3RD
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            box_features_2nd, _ = self.roi_heads.box_head_2nd(box_features)
            embeddings_2nd, _ = self.roi_heads.embedding_head_2nd(box_features_2nd)
            box_features_3rd, _ = self.roi_heads.box_head_3rd(box_features)
            embeddings_3rd, _ = self.roi_heads.embedding_head_3rd(box_features_3rd)
            box_features_4th = self.roi_heads.box_head_4th(box_features)
            embeddings_4th, _ = self.roi_heads.embedding_head_4th(box_features_4th)
            ...
            elif self.eval_feat == 'stage4':
                embeddings = embeddings_4th
            else:
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        _, rcnn_losses, feats_reid_2nd, targets_reid_2nd, feats_reid_3rd, targets_reid_3rd \
        ...
        losses["loss_rcnn_reid_4th"] *= self.lw_rcnn_reid_4th #
...
class CascadedROIHeads(RoIHeads):
    ...
    def __init__(
        ...
        self.box_head_4th = box_head_4th
        ...
        self.embedding_head_4th = deepcopy(self.embedding_head_2nd)

        self.fuse_embedding_23 = information_flow(256)
        self.fuse_embedding_34 = deepcopy(self.fuse_embedding_23)
        # OIM
        ...
        self.reid_loss_2nd = LOIMLoss(embedding_dim, num_pids, num_cq_size, oim_momentum, oim_scalar, 0.1)
        self.reid_loss_3rd = deepcopy(self.reid_loss_2nd)
        self.reid_loss_4th = OIMLoss(embedding_dim, num_pids, num_cq_size, oim_momentum, oim_scalar)
    ...
    def forward(self, features, boxes, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        box_features_1st, _ = self.box_head_1st(box_features_1st)
        ...
        if self.training:
            ...
            box_target = torch.cat(box_target, dim=0)
            # ious_2nd = torch.clamp(ious, min=0.7)
            loss_giou_1st = 1 - torch.diag(box.generalized_box_iou(
                box.box_cxcywh_to_xyxy(box_pred),
                box.box_cxcywh_to_xyxy(box_target)))
            loss_giou_1st = loss_giou_1st.sum() / box_pred.numel()
        ...
        if self.training:
            ...
            # box_cls_scores_1st = (box_cls_scores_1st + box_cls_scores_1st_) - torch.mul(box_cls_scores_1st_, box_cls_scores_1st)
        ...
        if not self.training and query_img_as_gallery:
            ...
            gt_box_features, _ = self.box_head_2nd(gt_box_features)
        ...
        if boxes[0].shape[0] == 0:
            ...
            if self.eval_feat == 'concat':
                embeddings = torch.cat((gt_det_2nd["embeddings"], gt_det_2nd["embeddings"]),
                                       dim=1) if gt_det_2nd else torch.zeros(0, 512)
            elif self.eval_feat == 'stage2' or self.eval_feat == 'stage3' or self.eval_feat=='stage4':
        ...
        box_features, y_reg = self.box_head_2nd(box_features)

        box_regs_2nd = self.box_predictor_2nd(y_reg)
        if self.training: register_targets_for_pn(self.embedding_head_2nd, torch.cat(box_pid_labels_2nd).long())
        box_embeddings_2nd, _ = self.embedding_head_2nd(box_features)
        if self.training: register_targets_for_pn(self.embedding_head_2nd, torch.cat(box_pid_labels_2nd).long())
        _, box_cls_scores_2nd = self.embedding_head_2nd(box_features)
        # box_cls_scores_2nd, box_regs_2nd = self.box_predictor_2nd(y_reg)
        # box_cls_scores_2nd = box_cls_scores_2nd.squeeze(1)
        # box_regs_2nd = box_regs_2nd.repeat(1, 2)
        # box_embeddings_2nd, _ = self.embedding_head_2nd(box_features)
        ...
        if self.training:
            ...
            box_target = torch.cat(box_target, dim=0)
            ious_2nd = torch.clamp(ious, min=0.7)
            loss_giou_2nd = 1 - torch.diag(box.generalized_box_iou(
                box.box_cxcywh_to_xyxy(box_pred),
                box.box_cxcywh_to_xyxy(box_target)))
            loss_giou_2nd = loss_giou_2nd.sum() / box_pred.numel()
        ...
        if not self.training and query_img_as_gallery and self.eval_feat != 'stage2':
            ...
            gt_box = [targets[0]["boxes"]]
            gt_box_features = self.box_roi_pool(features, gt_box, image_shapes)
            gt_box_features, _ = self.box_head_3rd(gt_box_features)
        ...
        if boxes[0].shape[0] == 0 and self.eval_feat != 'stage2':
            ...
            if self.eval_feat == 'concat':
                embeddings = torch.cat((gt_det_2nd["embeddings"], gt_det_3rd["embeddings"]),
                                       dim=1) if gt_det_3rd else torch.zeros(0, 512)
            elif self.eval_feat == 'stage3' or self.eval_feat=='stage4':
        ...
        if self.training:
            with torch.no_grad():
                box_features_2nd_, _ = self.box_head_2nd(box_features)
                register_targets_for_pn(self.embedding_head_2nd, torch.cat(box_pid_labels_3rd).long())
                box_embeddings_2nd_, _ = self.embedding_head_2nd(box_features_2nd_)
        else:
             box_features_2nd, _ = self.box_head_2nd(box_features)
             box_embeddings_2nd, _ = self.embedding_head_2nd(box_features_2nd)

        box_features, y_reg = self.box_head_3rd(box_features)
        box_regs_3rd = self.box_predictor_3rd(y_reg)
        if self.training: register_targets_for_pn(self.embedding_head_3rd, torch.cat(box_pid_labels_3rd).long())
        box_embeddings_3rd, _ = self.embedding_head_3rd(box_features)
        if self.training: register_targets_for_pn(self.embedding_head_3rd, torch.cat(box_pid_labels_3rd).long())
        _, box_cls_scores_3rd = self.embedding_head_3rd(box_features)

        # box_cls_scores_3rd, box_regs_3rd = self.box_predictor_3rd(y_reg)
        # box_cls_scores_3rd = box_cls_scores_3rd.squeeze(1)
        # box_regs_3rd = box_regs_3rd.repeat(1, 2)
        # if self.training : register_targets_for_pn(self.embedding_head_3rd, torch.cat(box_pid_labels_3rd).long())
        # box_embeddings_3rd, _ = self.embedding_head_3rd(box_features)
        if self.training:
            # register_targets_for_pn(self.fuse_embedding_23, torch.cat(box_pid_labels_3rd).long())
            box_embeddings_3rd = self.fuse_embedding_23(box_embeddings_2nd_, box_embeddings_3rd)
        else:
            box_embeddings_3rd = self.fuse_embedding_23(box_embeddings_2nd, box_embeddings_3rd)
        if self.training:
            max_iou_list = []
            # step1. compute IoU between all proposals and all ground-truth boxes
            # step2. select the maximum ground-truth box for each proposal
            ...
            ious_3rd = torch.clamp(ious, min=0.7)
            loss_giou_3rd = 1 - torch.diag(box.generalized_box_iou(
                box.box_cxcywh_to_xyxy(box_pred),
                box.box_cxcywh_to_xyxy(box_target)))
            loss_giou_3rd = loss_giou_3rd.sum() / box_pred.numel()

        if box_cls_scores_3rd.dim() == 0:
            box_cls_scores_3rd = box_cls_scores_3rd.unsqueeze(0)
        # add fuse feat and fourth stage
        ...
        if self.training:
            ...
            if self.use_diff_thresh:
                self.proposal_matcher = det_utils.Matcher(
                    ...
                    self.bg_iou_thresh_3rd,  #
                    allow_low_quality_matches=False)
            boxes, _, box_pid_labels_4th, box_reg_targets_4th = self.select_training_samples(boxes, targets)
        ...
        if not self.training and query_img_as_gallery and self.eval_feat != 'stage2':
            ...
            gt_box = [targets[0]["boxes"]]
            gt_box_features = self.box_roi_pool(features, gt_box, image_shapes)
            gt_box_features = self.box_head_4th(gt_box_features)
            embeddings, _ = self.embedding_head_4th(gt_box_features)
            gt_det_4th = {"boxes": targets[0]["boxes"], "embeddings": embeddings}

        # no detection predicted by Faster R-CNN head in test phase
        if boxes[0].shape[0] == 0 and self.eval_feat != 'stage2':
            assert not self.training
            boxes = gt_det_4th["boxes"] if gt_det_4th else torch.zeros(0, 4)
            labels = torch.ones(1).type_as(boxes) if gt_det_4th else torch.zeros(0)
            scores = torch.ones(1).type_as(boxes) if gt_det_4th else torch.zeros(0)
            if self.eval_feat == 'concat':
                embeddings = torch.cat((gt_det_3rd["embeddings"], gt_det_4th["embeddings"]),
                                       dim=1) if gt_det_4th else torch.zeros(0, 512)
            elif self.eval_feat == 'stage4':
                embeddings = gt_det_3rd["embeddings"] if gt_det_4th else torch.zeros(0, 256)
            else:
                raise Exception("Unknown evaluation feature name")
            return [dict(boxes=boxes, labels=labels, scores=scores, embeddings=embeddings)], []
        # --------------------- The fourth stage -------------------- #
        ...
        if self.training:
            with torch.no_grad():
                box_features_3rd_, _ = self.box_head_3rd(box_features)
                register_targets_for_pn(self.embedding_head_3rd, torch.cat(box_pid_labels_4th).long())
                box_embeddings_3rd_, _ = self.embedding_head_3rd(box_features_3rd_)
        else:
             box_features_3rd, _ = self.box_head_3rd(box_features)
             box_embeddings_3rd, _ = self.embedding_head_3rd(box_features_3rd)
        box_features = self.box_head_4th(box_features)

        if self.training: register_targets_for_pn(self.embedding_head_4th, torch.cat(box_pid_labels_4th).long())
        box_embeddings_4th, _ = self.embedding_head_4th(box_features)

        if self.training:
            # register_targets_for_pn(self.fuse_embedding_34, torch.cat(box_pid_labels_3rd).long())
            box_embeddings_4th = self.fuse_embedding_34(box_embeddings_3rd_, box_embeddings_4th)
        else:
            box_embeddings_4th = self.fuse_embedding_34(box_embeddings_3rd, box_embeddings_4th)
        if self.training:
            max_iou_list = []
            # step1. compute IoU between all proposals and all ground-truth boxes
            # step2. select the maximum ground-truth box for each proposal
            ...
            ious = torch.cat(max_iou_list, dim=0)
            ious_4th = torch.clamp(ious, min=0.7)
        ...
        if self.training:
            ...
            losses['loss_rcnn_reg_1st'] = losses['loss_rcnn_reg_1st'] + loss_giou_1st
            losses['loss_rcnn_reg_2nd'] = losses['loss_rcnn_reg_2nd'] + loss_giou_2nd
            losses['loss_rcnn_reg_3rd'] = losses['loss_rcnn_reg_3rd'] + loss_giou_3rd
            loss_rcnn_reid_2nd, feats_reid_2nd, targets_reid_2nd = self.reid_loss_2nd(
                box_embeddings_2nd, box_pid_labels_2nd, ious_2nd)
            loss_rcnn_reid_3rd, feats_reid_3rd, targets_reid_3rd = self.reid_loss_3rd(
                box_embeddings_3rd, box_pid_labels_3rd, ious_3rd)
            loss_rcnn_reid_4th, feats_reid_4th, targets_reid_4th = self.reid_loss_4th(
                box_embeddings_4th, box_pid_labels_4th)
            losses.update(loss_rcnn_reid_2nd=loss_rcnn_reid_2nd)
            losses.update(loss_rcnn_reid_3rd=loss_rcnn_reid_3rd)
            losses.update(loss_rcnn_reid_4th=loss_rcnn_reid_4th)
        else:
            if self.eval_feat == 'stage2':
                ...
                _,_,embeddings_4th,_ = self.postprocess_boxes(
                    box_cls_scores_3rd,
                    box_regs_3rd,
                    box_embeddings_4th,
                    boxes,
            ...
            for i in range(num_images):
                if self.eval_feat == 'concat':
                    embeddings = torch.cat((embeddings_3rd[i], embeddings_4th[i]), dim=1)
                elif self.eval_feat == 'stage2':
                    embeddings = embeddings_2nd[i]
                elif self.eval_feat == 'stage3':
                    embeddings = embeddings_3rd[i]
                elif self.eval_feat == 'stage4':
                    embeddings = embeddings_4th[i]
                else:
                    raise Exception("Unknown evaluation feature name")
                result.append(
                    dict(
...
class information_flow(nn.Module):
    def __init__(self, in_chans):
        super().__init__()
        self.fc1 = nn.Sequential(nn.Linear(in_chans, in_chans), nn.BatchNorm1d(in_chans))
        init.normal_(self.fc1[0].weight, std=0.01)
        init.constant_(self.fc1[0].bias, 0)

        self.fc2 = nn.Sequential(nn.Linear(in_chans, in_chans), nn.BatchNorm1d(in_chans))
        init.normal_(self.fc2[0].weight, std=0.01)
        init.constant_(self.fc2[0].bias, 0)

        self.fc3 = nn.Sequential(nn.Linear(in_chans, in_chans), nn.BatchNorm1d(in_chans))
        init.normal_(self.fc3[0].weight, std=0.01)
        init.constant_(self.fc3[0].bias, 0)

        self.fc4 = nn.Sequential(nn.Linear(in_chans, in_chans), nn.BatchNorm1d(in_chans))
        init.normal_(self.fc4[0].weight, std=0.01)
        init.constant_(self.fc4[0].bias, 0)

        self.act = nn.GELU()

    def forward(self, pred_feat, feat):
        pred_feat = self.act(self.fc1(pred_feat))
        feat = 0.1*pred_feat+0.9*feat
        feat = self.act(self.fc2(feat))
        feat = self.act(self.fc3(feat))
        feat = self.act(self.fc4(feat))
        return feat
...
class NormAwareEmbedding(nn.Module):
    ...
    def __init__(self, featmap_names=["feat_res4", "feat_res5"], in_channels=[1024, 2048], dim=256, mode='pn'):
...
class BBoxRegressor(nn.Module):
    ...
    def __init__(self, in_channels, num_classes=2, bn_neck=True, mode='ln'):
        """
        Args:
...
def detection_losses(
    ...
    # loss_rcnn_cls_2nd = focal_loss.sigmoid_focal_loss(box_cls_scores_2nd, box_labels_2nd.float(), reduction='mean')
    ...
    # loss_rcnn_cls_3rd = focal_loss.sigmoid_focal_loss(box_cls_scores_3rd, box_labels_3rd.float(), reduction='mean')
