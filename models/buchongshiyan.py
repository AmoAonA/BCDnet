...
class COAT(nn.Module):
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        losses["loss_rcnn_reg_4th"] *= self.lw_rcnn_reg_3rd  # u
        losses["loss_rcnn_cls_4th"] *= self.lw_rcnn_cls_3rd  # u
...
class CascadedROIHeads(RoIHeads):
    ...
    def __init__(
        ...
        self.box_predictor_4th = deepcopy(self.box_predictor)
        # Transformer head
    ...
    def forward(self, features, boxes, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        # box_cls_scores_1st_, box_regs_1st_ = self.box_predictor_1st_(F.adaptive_avg_pool2d(y_reg, 1))
        ...
        box_features, y_4th = self.box_head_4th(box_features)
        box_regs_4th = self.box_predictor_4th(y_4th)
...
def detection_losses(
    ...
    box_reg_targets_4th = torch.cat(box_reg_targets_4th, dim=0)
    loss_rcnn_cls_4th = F.binary_cross_entropy_with_logits(box_cls_scores_4th, box_labels_4th.float())

    sampled_pos_inds_subset = torch.nonzero(box_labels_4th > 0).squeeze(1)
    labels_pos = box_labels_4th[sampled_pos_inds_subset]
    N = box_cls_scores_4th.size(0)
    box_regs_4th = box_regs_4th.reshape(N, -1, 4)

    loss_rcnn_reg_4th = F.smooth_l1_loss(
        box_regs_4th[sampled_pos_inds_subset, labels_pos],
        box_reg_targets_4th[sampled_pos_inds_subset],
        reduction="sum",
    )
    loss_rcnn_reg_4th = loss_rcnn_reg_4th / box_labels_4th.numel()
    return dict(
        ...
        loss_rcnn_reg_4th=loss_rcnn_reg_4th,
    )
