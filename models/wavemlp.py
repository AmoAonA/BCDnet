...
class WaveBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = PATM(dim, qkv_bias=qkv_bias, qk_scale=None, attn_drop=attn_drop, mode=mode)  # token mixer
...
class Vec2Patch(nn.Module):
    def __init__(self, hidden, in_chans, output_size, kernel_size, stride, padding):
...
class Wave_block(nn.Module):
    def __init__(self, cfg, in_chans, embed_dim, kernel_size, padding, stride, use_feature_mask):
        ...
        self.vec2patch = Vec2Patch(embed_dim, in_chans, output_size, kernel_size, stride, padding)
...
class ConverterNet(nn.Module):
    def __init__(self, in_channel):
        super(ConverterNet, self).__init__()
        self.g_conv1 = nn.Conv2d(in_channel, in_channel // 2, 3, 1, 1, groups=2, bias=False)
        self.norm1 = nn.BatchNorm2d(in_channel // 2)
        self.conv1 = nn.Conv2d(in_channel // 2, in_channel // 2, 1, 1, 0, bias=False)
        self.g_conv2 = nn.Conv2d(in_channel // 2, in_channel // 4, 3, 1, 1, groups=2, bias=False)
        self.norm2 = nn.BatchNorm2d(in_channel // 4)
        self.conv2 = nn.Conv2d(in_channel // 4, in_channel // 4, 1, 1, 0, bias=False)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        x = self.relu(self.norm1(self.g_conv1(x)))
        x = self.conv1(x)
        x = self.relu(self.norm2(self.g_conv2(x)))
        x = self.conv2(x)
        return F.adaptive_avg_pool2d(x, 1)


class CenterFocus(nn.Module):
    def __init__(self, in_channel):
        super(CenterFocus, self).__init__()
        self.Converter_center = ConverterNet(in_channel)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
            nn.Linear(in_channel, in_channel // 16, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(in_channel // 16, in_channel, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        b, c, _, _ = x.shape
        x_center = x[:, :, :, 5:7]
        y = self.avg_pool(x_center).view(b, c)
        y = self.fc(y).view(b, c, 1, 1)
        x = x * y.expand_as(x)
        x = self.Converter_center(self.act(x))  # +x
        return x


class Relational_Adaptive(nn.Module):
    def __init__(self, in_channel):
        super(Relational_Adaptive, self).__init__()
        self.mlp = Mlp(in_features=in_channel + in_channel // 4, hidden_features=in_channel,
                       out_features=in_channel // 4)

    def forward(self, feat_list, x):
        x = F.adaptive_avg_pool2d(x, 1)
        new_list = []
        for feat in feat_list:
            new_list.append(torch.concat([x, feat], dim=1))
        feat = torch.concat(new_list, dim=2)
        feat = self.mlp(feat)
        b, c, h, w = feat.shape
        return feat.view(b, c * 4, h // 4, 1)


class RegionPoolingNet(nn.Module):
    def __init__(self, in_channel):
        super(RegionPoolingNet, self).__init__()
        self.Converter_top = ConverterNet(in_channel)
        self.Converter_bottom = ConverterNet(in_channel)
        self.Converter_global = ConverterNet(in_channel)
        self.center = CenterFocus(in_channel)
        self.ram = Relational_Adaptive(in_channel)

    def forward(self, x):
        feat_list = []
        x_top, x_bottom = torch.chunk(x, 2, 2)
        # list = [global , top , bottom , center]
        feat_list.append(self.Converter_global(x))
        feat_list.append(self.Converter_top(x_top))
        feat_list.append(self.Converter_bottom(x_bottom))
        feat_list.append(self.center(x))
        para = self.ram(feat_list, x)
        all_feat = torch.concat(feat_list, dim=1)
        return torch.matmul(para, all_feat)


class RegionPoolingNet_new(nn.Module):
    def __init__(self, in_channel):
        super(RegionPoolingNet_new, self).__init__()
        self.Converter_top = ConverterNet(in_channel)
        self.Converter_bottom = ConverterNet(in_channel)
        self.Converter_global = ConverterNet(in_channel)
        self.center = CenterFocus(in_channel)
    ...
    def forward(self, x):
        ...
        para = self.ram(feat_list, x)
        all_feat = []
        for x, y in feat_list, torch.chunk(para, 4, 1):
...
class WaveHead_two(nn.Module):
    def __init__(self, cfg, block_name, kernel_size, use_feature_mask):
        ...
        self.early_conv = nn.Sequential(nn.BatchNorm2d(1024),
                                        nn.Conv2d(1024, dim, 3, 1, 1),
                                        SELayer(dim),
                                        nn.Conv2d(dim, dim, 1, 1, 0)
                                        )
        self.conv1 = nn.Conv2d(1024, dim, 1, 1, 0)
        self.conv2 = nn.Conv2d(dim, 2048, 1, 1, 0)
        self.pooling = RegionPoolingNet(dim)

    def forward(self, x):
        mask = self.get_mask_box(x)
        if self.use_feature_mask:
            skip_feat = self.skip(x)  # 先卷积
            if self.training:
                skip_feat = self.bypass_mask(skip_feat)  # 交换patch
        else:
            skip_feat = x
        feats = {}
        y = self.reg_head(x)  # only-x
        y = F.adaptive_max_pool2d(y, 1)  # 是否池化?
        feats['before_trans'] = F.adaptive_max_pool2d(skip_feat, 1)
        x = self.conv1(x)+ self.early_conv(x)
        x = self.encoder((x, mask))
        x = self.conv2(self.pooling(x))
        feats['after_trans'] = x  # F.adaptive_max_pool2d(x, 1)
        return feats, y
