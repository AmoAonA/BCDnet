...
from loss.oim2 import newOIMLoss, OIMLoss
from models.backbone.backbone_resnet import build_resnet
from models.wavemlp_aug import WaveHead_two
from models.custom_modules import PrototypeNorm1d, register_targets_for_pn
...
class COAT(nn.Module):
    def __init__(self, cfg):
        ...
        box_predictor = BBoxRegressor(1024, num_classes=2, bn_neck=cfg.MODEL.ROI_HEAD.BN_NECK)
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            box_features_4th, _ = self.roi_heads.box_head_4th(box_features)
            embeddings_4th, _ = self.roi_heads.embedding_head_4th(box_features_4th)
            if self.eval_feat == 'concat':
                embeddings = torch.cat((embeddings_2nd, embeddings_3rd, embeddings_4th), dim=1)
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        feats_reid_4th, targets_reid_4th = self.roi_heads(features, boxes, images.image_sizes, targets)
        ...
        losses["loss_rcnn_reid_4th"] *= self.lw_rcnn_reid_3rd
        return losses, feats_reid_2nd, targets_reid_2nd, feats_reid_3rd, targets_reid_3rd, feats_reid_4th, targets_reid_4th
...
class CascadedROIHeads(RoIHeads):
    ...
    def __init__(
        ...
        self.reid_loss_2nd = newOIMLoss(embedding_dim, num_pids, num_cq_size, oim_momentum, oim_scalar,0.1)
    ...
    def forward(self, features, boxes, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        if self.training:
            ...
            ious_1st = self.get_IoU(boxes, targets)
        else:
        ...
        box_embeddings_2nd, box_cls_scores_2nd = self.embedding_head_2nd(box_features,box_pid_labels_2nd)
        ...
        if self.training:
            ...
            boxes = [boxes_per_image.detach() for boxes_per_image in boxes]
            ious_2nd_corr = self.get_IoU(boxes, targets)
            if self.use_diff_thresh:
                self.proposal_matcher = det_utils.Matcher(
            ...
            ious_2nd = self.get_IoU(boxes, targets)  # ious_2nd 和 box_embedding_2nd 有关
        else:
        ...
        # --------------------- The third stage -------------------- #
        box_features = self.box_roi_pool(features, boxes, image_shapes)
        box_features, y_3rd = self.box_head_3rd(box_features)
        box_regs_3rd = self.box_predictor_3rd(y_3rd)
        if self.training: register_targets_for_pn(self.embedding_head_3rd, torch.cat(box_pid_labels_3rd).long())
        box_embeddings_3rd,box_cls_scores_3rd = self.embedding_head_3rd(box_features,box_pid_labels_3rd)
        ...
        if self.training:
            ...
            boxes = [boxes_per_image.detach() for boxes_per_image in boxes]
            ious_3rd_corr = self.get_IoU(boxes, targets)
            if self.use_diff_thresh:
                self.proposal_matcher = det_utils.Matcher(
                    self.fg_iou_thresh_3rd,  #
                    self.bg_iou_thresh_3rd,  #
            ...
            ious_3rd = self.get_IoU(boxes, targets)
        else:
            self.nms_thresh = self.nms_thresh_3rd
            if self.eval_feat != 'stage3':
                boxes, scores, _, _ = self.postprocess_boxes(
                    box_cls_scores_3rd,
                    box_regs_3rd,
                    box_embeddings_3rd,
        ...
        if not self.training and query_img_as_gallery and self.eval_feat != 'stage2':
            ...
            gt_box = [targets[0]["boxes"]]
            gt_box_features = self.box_roi_pool(features, gt_box, image_shapes)
            gt_box_features, _ = self.box_head_4th(gt_box_features)
        ...
        box_features, _ = self.box_head_4th(box_features)
        if self.training: register_targets_for_pn(self.embedding_head_4th, torch.cat(box_pid_labels_4th).long())
        box_embeddings_4th,box_cls_scores_4th = self.embedding_head_4th(box_features,box_pid_labels_4th)

        if box_cls_scores_4th.dim() == 0:
            box_cls_scores_4th = box_cls_scores_4th.unsqueeze(0)
        ...
        if self.training:
            ...
            # box_labels_4th = [y.clamp(0, 1) for y in box_pid_labels_4th]
            ...
            losses['loss_rcnn_reg_1st'] = losses['loss_rcnn_reg_1st']  # + giou_loss_1st
            losses['loss_rcnn_reg_2nd'] = losses['loss_rcnn_reg_2nd']  # + giou_loss_2nd
            losses['loss_rcnn_reg_3rd'] = losses['loss_rcnn_reg_3rd']  # + giou_loss_3rd
            # reid 和 iou统一阶段预测 是否使用其预测结果做为动量呢？我认为可以，但原论文使用rpn的bbox来进行筛
            # 待测试
            # ious_2nd = torch.clamp(ious_2nd, min=self.bg_iou_thresh_2nd)
            # print()
            loss_rcnn_reid_2nd, feats_reid_2nd, targets_reid_2nd = self.reid_loss_2nd(
                box_embeddings_2nd, box_pid_labels_2nd,ious_2nd_corr,box_cls_scores_2nd)
            loss_rcnn_reid_3rd, feats_reid_3rd, targets_reid_3rd = self.reid_loss_3rd(
                box_embeddings_3rd, box_pid_labels_3rd,ious_3rd_corr,box_cls_scores_3rd)
            ...
            if self.eval_feat == 'stage2':
                ...
                self.nms_thresh = self.nms_thresh_3rd
                _, _, embeddings_2nd, _ = self.postprocess_embeddings(
                    box_cls_scores_4th,
                    box_embeddings_2nd,
                    boxes,
                    image_shapes,
                ...
                _, _, embeddings_3rd, _ = self.postprocess_embeddings(
                    box_cls_scores_4th,
                    box_embeddings_3rd,
                    boxes,
                    image_shapes,
                ...
                boxes, scores, embeddings_4th, labels = self.postprocess_embeddings(
                    box_cls_scores_4th,
                    box_embeddings_4th,
                    boxes,
                    image_shapes,
            ...
            for i in range(num_images):
                if self.eval_feat == 'concat':
                    embeddings = torch.cat((embeddings_2nd[i], embeddings_3rd[i], embeddings_4th[i]), dim=1)
                elif self.eval_feat == 'stage2':
        ...
        return result, losses, feats_reid_2nd, targets_reid_2nd, feats_reid_3rd, targets_reid_3rd, feats_reid_4th, targets_reid_4th
    ...
    def postprocess_embeddings(
        ...
        boxes_per_image = [len(boxes_in_image) for boxes_in_image in proposals]
        box_regression = torch.zeros((boxes_per_image[0], 8), device=device)
        pred_boxes = self.box_coder.decode(box_regression, proposals)
    ...
    def get_IoU(self, boxes, targets):
        max_iou_list = []
        for batch_index in range(len(boxes)):
            box_p = boxes[batch_index]
            # assert box_p.shape[0] % 128 == 0, 'box:{}'.format(box_p.shape)
...
class NormAwareEmbedding(nn.Module):
    ...
    def forward(self, featmaps,labels=None):
        """
        Arguments:
        ...
        if len(featmaps) == 1:
            ...
            norms = embeddings.norm(2, 1, keepdim=True)
            if self.training:
                labels = torch.cat(labels).detach()
                targets_unique = torch.unique(labels)
                for t in targets_unique:
                    if t != 5555:
                        temp = embeddings[labels == t]
                        prototypes = (temp.mean([0])).norm(2, 0, keepdim=True)
                        for i in range(temp.shape[0]):
                            a = temp[i]/prototypes.expand_as(temp[i]).clamp(min=1e-12)
                            temp[i] =a 
            else:
                embeddings = embeddings / norms.expand_as(embeddings).clamp(min=1e-12)
                norms = self.rescaler(norms).squeeze()
...
class pnNormAwareEmbedding(nn.Module):
    """
    Implements the Norm-Aware Embedding proposed in
    Chen, Di, et al. "Norm-aware embedding for efficient person search." CVPR 2020.
    """
    ...
    def __init__(self, featmap_names=["feat_res4", "feat_res5"], in_channels=[1024, 2048], dim=256, mode='pn'):
        super(pnNormAwareEmbedding, self).__init__()
        self.featmap_names = featmap_names
        self.in_channels = in_channels
        self.dim = dim
        if mode == 'ln':
        ...
        #     init.constant_(proj[0].bias, 0)
        #     init.constant_(proj[1].bias, 0)
        #     self.projectors2[ftname] = proj
        #
        for ftname, in_channel,indv_dim in zip(self.featmap_names,self.in_channels,indv_dims):
            proj = nn.Linear(in_channel,indv_dim)
            norm1 = norm_layer(indv_dim)
            norm2 = nn.BatchNorm1d(indv_dim)
            init.normal_(proj.weight, std=0.01)
            init.normal_(norm2.weight, std=0.01)
            init.constant_(proj.bias, 0)
            init.constant_(norm2.bias, 0)
            self.projectors[ftname] = nn.Sequential(proj,norm1)
            self.projectors2[ftname] = nn.Sequential(proj,norm2)

        self.rescaler = norm_layer(1)
        self.rescaler1 = nn.BatchNorm1d(1)
    ...
    def forward(self, featmaps):
        ...
        if len(featmaps) == 1:
            ...
            outputs_1,outputs_2 = [],[]
            for k, v in featmaps.items():
                v_1 = self._flatten_fc_input(v)
                v_2 = self._flatten_fc_input(v)
                outputs_1.append(self.projectors[k](v_1))
                outputs_2.append(self.projectors2[k](v_2))

            embeddings_1,embeddings_2 = torch.cat(outputs_1, dim=1),torch.cat(outputs_2,dim=1)
            norms_1,norms_2 = embeddings_1.norm(2, 1, keepdim=True),embeddings_2.norm(2,1,keepdim=True)

            embeddings_1,embeddings_2 = embeddings_1 / norms_1.expand_as(embeddings_1).clamp(min=1e-12),embeddings_2 / norms_2.expand_as(embeddings_2).clamp(min=1e-12)
            norms_1,norms_2 = self.rescaler(norms_1).squeeze(),self.rescaler1(norms_2).squeeze()
...
def detection_losses(
    ...
    # 0.1 * F.cross_entropy(box_cls_scores_1st_, box_labels_1st)
    ...
    # box_regs_1st_ = box_regs_1st_.reshape(N, -1, 4)
    # 待换成GIOU
    ...
    # box_labels_4th = torch.cat(box_labels_4th, dim=0)
    # loss_rcnn_cls_4th = F.binary_cross_entropy_with_logits(box_cls_scores_4th, box_labels_4th.float())
    return dict(
        ...
        # loss_rcnn_cls_4th=loss_rcnn_cls_4th,
    )