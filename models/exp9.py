...
from loss.selective_loss import Loss_DC_base
from loss.selective_loss import Distance_Correlation
...
class COAT(nn.Module):
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        # losses["loss_corr_2nd"] *= 0.1
        # losses["loss_corr_3rd"] *= 0.1
...
class CascadedROIHeads(RoIHeads):
    ...
    def __init__(
        ...
        self.corr_2nd = Loss_DC_base()
        self.corr_3rd = deepcopy(self.corr_2nd)
        # evaluation
        self.eval_feat = cfg.EVAL_FEATURE
    ...
    def forward(self, features, boxes, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        if self.training: register_targets_for_pn(self.embedding_head_4th, torch.cat(box_pid_labels_4th).long())
        _, box_cls_scores_4th = self.embedding_head_4th(box_features)
        ...
        if self.training:
            ...
            loss_rcnn_reid_2nd, feats_reid_2nd, targets_reid_2nd = self.reid_loss_2nd(
                box_embeddings_2nd, box_pid_labels_2nd)
            # 预测的box_1st得到的特征embedding2和reg2 因此iou 1st和embedding2同reg2 公用一个 pid2
            # 我需要一个和pid3对应的iou
            loss_rcnn_reid_3rd, feats_reid_3rd, targets_reid_3rd = self.reid_loss_3rd(
                box_embeddings_3rd, box_pid_labels_3rd)
            loss_rcnn_reid_4th, feats_reid_4th, targets_reid_4th, = self.reid_loss_4th(
                box_embeddings_4th, box_pid_labels_4th)
            # 预测的box_1st得到的特征embedding2和reg2 因此iou 1st和embedding2同reg2 公用一个 pid2
            # 我需要一个和pid3对应的iou
            corr_2nd = self.corr_2nd(target=box_embeddings_2nd.clone(),
                                     ious=ious_2nd_corr.clone(),
                                     cls=box_cls_scores_2nd.clone(),
                                     label=box_pid_labels_2nd).detach()
            # corr_2nd = self.corr_2nd(box_embeddings_2nd.detach(), ious_2nd_corr.detach(), box_pid_labels_2nd)
            # embeding2nd 和 reg2nd对应和pid2nd对应 需要在get_box后直接获得相关iou
            corr_3rd = self.corr_3rd(target=box_embeddings_3rd.clone(),
                                     ious=ious_3rd_corr.clone(),
                                     cls=box_cls_scores_3rd.clone(),
                                     label=box_pid_labels_3rd).detach()

            losses.update(loss_corr_2nd=corr_2nd)
            losses.update(loss_corr_3rd=corr_3rd)
    ...
    # def get_IoU(self, boxes, targets, index):
    #     box_pred, box_target, ious_list = [], [], []
    #     for batch_index in range(len(boxes)):
    #         idx = (index[batch_index] != 0)
    #         box_p = boxes[batch_index][idx]
    #         box_t = targets[batch_index]['boxes']
    #         ious = box_ops.box_iou(box_p, box_t)  # pos_pred
    #         ious_max = torch.max(ious, dim=1)[0]
    #         box_pred.append(box_p)
    #         box_target.append(box_t)
    #         ious_list.append(ious_max)
    #     ious = torch.cat(ious_list, dim=0)
    #     box_pred = torch.cat(box_pred, dim=0)
    #     box_target = torch.cat(box_target, dim=0)
    #     loss_giou = 1 - torch.diag(box.generalized_box_iou(box_pred, box_target))
    #     loss_giou = loss_giou.sum() / box_pred.numel()
