...
class NonLocalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        b, c, h, w = x.size()
        # [N, C/2, H * W]
        x_phi = self.conv_phi(x).view(b, c//2,h*w)
        # [N, H * W, C/2]
        x_theta = self.conv_theta(x).view(b, c//2, h*w).permute(0, 2, 1).contiguous()
        x_g = self.conv_g(x).view(b, c//2, h*w).permute(0, 2, 1).contiguous()
        # [N, H * W, H * W]
...
class Wave_block(nn.Module):
    def __init__(self, cfg, in_chans, embed_dim, kernel_size, padding, stride, use_feature_mask):
        ...
        self.wave_mlp = basic_blocks(dim=embed_dim, num_layers=num_layers, drop_path_rate=dropout)
        self.fc1 = nn.Conv2d(in_chans, embed_dim, 1, 1, 0)
        self.fc2 = nn.Conv2d(embed_dim,in_chans,1,1,0)
    ...
    def forward(self, x):
        ...
        trans_feat = self.fc2(self.wave_mlp(trans_feat))
    ...
    def __init__(self, cfg, blocks_name, kernel_list, use_feature_mask):
        ...
        for name, kernel, stride, padding in zip(self.blocks_name, kernel_list,
            ...
            block = Wave_block(cfg, in_chans=dim // self.scale_size,  # 512//2
                               embed_dim=dim // (2 * self.scale_size),  # 512//4
                               kernel_size=kernel,
                               stride=stride,
                               padding=padding,
...
class WaveHead_two(nn.Module):
    def __init__(self, cfg, block_name, kernel_size, use_feature_mask):
        ...
        self.bypass_mask = exchange_patch(mask_shape, mask_size, mask_mode)
        self.get_mask_box = get_mask_box(mask_shape, mask_size, mask_mode)
        self.reg_head = NonLocalBlock(2048)
        self.encoder = Wave_blocks(
            cfg=cfg,
            blocks_name=block_name,
            kernel_list=kernel_size,
        ...
        self.conv1 = nn.Conv2d(1024, dim, 1, 1, 0)
        self.conv2 = nn.Conv2d(dim, 2048, 1, 1, 0)
        _,resnet5 = build_resnet("resnet50",pretrained=True)
        self.Conv5 = resnet5
