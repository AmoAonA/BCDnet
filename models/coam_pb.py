...
from models.wavemlp_des_pb import WaveHead_two, WaveHead_des  # my_baseline
...
class COAT(nn.Module):
    ...
    def inference(self, images, targets=None, query_img_as_gallery=False):
        ...
        if targets is not None and not query_img_as_gallery:
            ...
            box_features_2nd, _, _, _,_ = self.roi_heads.box_head_2nd(box_features)
            embeddings_2nd, _ = self.roi_heads.embedding_head_2nd(box_features_2nd)
            box_features_3rd, _, _, _,_ = self.roi_heads.box_head_3rd(box_features)
...
class CascadedROIHeads(RoIHeads):
    ...
    def forward(self, features, boxes, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        box_features_1st, y_1st = self.box_head_1st(box_features_1st)
        ...
        if not self.training and query_img_as_gallery:
            ...
            gt_box_features,_,_, _, _ = self.box_head_2nd(gt_box_features)
        ...
        box_features, y_2nd, p_reid_2nd, p_cls_2nd,mask_2nd = self.box_head_2nd(box_features)
        box_regs_2nd = self.box_predictor_2nd(y_2nd)

        temp = box_features['after_trans']
        box_features['after_trans'] = torch.mul(box_features['after_trans'], mask_2nd[0].view(1, 256, 1, 1).repeat(1, 8, 1, 1))
        box_embeddings_2nd, box_cls_scores_2nd = self.embedding_head_2nd(box_features)
        box_features['after_trans'] = temp
        box_features['after_trans'] = torch.mul(box_features['after_trans'], mask_2nd[1].view(1, 256, 1, 1).repeat(1, 8, 1, 1))
        box_embeddings_2nd_, box_cls_scores_2nd_ = self.embedding_head_2nd(box_features)

        if box_cls_scores_2nd.dim() == 0:
            box_cls_scores_2nd.unsqueeze_(0)
        if box_cls_scores_2nd_.dim() == 0:
            box_cls_scores_2nd_.unsqueeze_(0)
        box_embeddings_2nd = torch.stack((box_embeddings_2nd_, box_embeddings_2nd), dim=1)
        box_cls_scores_2nd = torch.stack((box_cls_scores_2nd_, box_cls_scores_2nd), dim=1).unsqueeze(dim=2)
        box_embeddings_2nd = torch.matmul(p_reid_2nd, box_embeddings_2nd).view(-1, 256)
        box_cls_scores_2nd = torch.matmul(p_cls_2nd, box_cls_scores_2nd).view(-1)
        ...
        if not self.training and query_img_as_gallery and self.eval_feat != 'stage2':
            ...
            gt_box = [targets[0]["boxes"]]
            gt_box_features = self.box_roi_pool(features, gt_box, image_shapes)
            gt_box_features, _, _, _,_ = self.box_head_3rd(gt_box_features)
        ...
        if not self.training:
            box_features_2nd, _, _, _,_ = self.box_head_2nd(box_features)
            box_embeddings_2nd, _ = self.embedding_head_2nd(box_features_2nd)  # 不知道是否要
        box_features, y_3rd, p_reid_3rd, p_cls_3rd,mask_3rd = self.box_head_3rd(box_features)
        box_regs_3rd = self.box_predictor_3rd(y_3rd)

        temp = box_features['after_trans']
        box_features['after_trans'] = torch.mul(box_features['after_trans'], mask_3rd[0].view(1, 256, 1, 1).repeat(1, 8, 1, 1))
        box_embeddings_3rd, box_cls_scores_3rd = self.embedding_head_3rd(box_features)
        box_features['after_trans'] = temp
        box_features['after_trans'] = torch.mul(box_features['after_trans'], mask_3rd[1].view(1, 256, 1, 1).repeat(1, 8, 1, 1))
        box_embeddings_3rd_, box_cls_scores_3rd_ = self.embedding_head_3rd(box_features)
        #if False in torch.eq(box_cls_scores_3rd, box_cls_scores_3rd_):
        #    print("bug")
        #    return
        if box_cls_scores_3rd.dim() == 0:
            box_cls_scores_3rd.unsqueeze_(0)
        if box_cls_scores_3rd_.dim() == 0:
            box_cls_scores_3rd_.unsqueeze_(0)
        box_embeddings_3rd = torch.stack((box_embeddings_3rd_, box_embeddings_3rd), dim=1)
        box_cls_scores_3rd = torch.stack((box_cls_scores_3rd_, box_cls_scores_3rd), dim=1).unsqueeze(dim=2)
        box_embeddings_3rd = torch.matmul(p_reid_3rd, box_embeddings_3rd).view(-1, 256)
        box_cls_scores_3rd = torch.matmul(p_cls_3rd, box_cls_scores_3rd).view(-1)
