...
class WaveHead_one(nn.Module):
    def __init__(self, cfg, block_name, kernel_size, use_feature_mask):
        super(WaveHead_one, self).__init__()
        dim = cfg.MODEL.TRANSFORMER.DIM_MODEL
        self.use_feature_mask = use_feature_mask
        mask_shape = cfg.MODEL.MASK_SHAPE
        mask_size = cfg.MODEL.MASK_SIZE
        ...
        for i in range(2):
            self.cls_score.append(nn.Linear(512, 2))
            self.bbox_pred.append(nn.Linear(512, 2 * 4))

        self.mlp_aa = Mlp(1024,out_features=dim)
        self.aa_cls = nn.Linear(dim, 2)
        self.aa_reg = nn.Linear(dim, 2)
        self.pobe = convfcModule(1024,1024)
    ...
    def forward(self, x):
        ...
        x_reg = self.conv2(x_reg)
        x_reg = F.adaptive_max_pool2d(x_reg, 1).view(x_reg.shape[0],-1)

        x_cls = self.conv1(x)
        x_cls = self.encoder((x_cls, mask))
        x_cls = F.adaptive_max_pool2d(x_cls, 1).view(x_cls.shape[0],-1)

        x_aa = F.adaptive_max_pool2d(self.mlp_aa(x),1).view(x.shape[0],-1)
        cls_score_s = []
        for i, mask_i in enumerate(mask_c):
            cls_score_s.append(self.cls_score[i](torch.mul(x_cls, mask_i)))
        cls_score = torch.cat(cls_score_s, dim=1)

        bbox_pred_s = []
        for i, mask_i in enumerate(mask_c):
            bbox_pred_s.append(self.bbox_pred[i](torch.mul(x_reg, mask_i)))
        bbox_pred = torch.cat(bbox_pred_s, dim=1)
        aa_cls_ = self.aa_cls(x_aa)
        aa_cls_ = F.softmax(aa_cls_.view((cls_score.shape[0], 2, -1)), dim=2)
        aa_cls = aa_cls_.view((cls_score.shape[0], -1))
        aa_cls = aa_cls / 2
        aa_reg_ = self.aa_reg(x_aa)
        aa_reg_ = F.softmax(aa_reg_.view((bbox_pred.shape[0], 2, -1)), dim=2)
        aa_reg = aa_reg_.view((bbox_pred.shape[0], -1))
        aa_reg = aa_reg / 2 # 2 是决策树

        return cls_score,bbox_pred,aa_cls,aa_reg


class convfcModule(nn.Module):
    def __init__(self,in_chans,out_chans):
        super(convfcModule, self).__init__()
