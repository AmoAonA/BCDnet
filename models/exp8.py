...
class COAT(nn.Module):
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        losses["loss_corr_2nd"] *= 1.
        losses["loss_corr_3rd"] *= 1.
...
class CascadedROIHeads(RoIHeads):
    ...
    def __init__(
        ...
        self.reid_loss_4th = deepcopy(self.reid_loss_2nd)
        self.corr_2nd = Loss_DC()
    ...
    def forward(self, features, boxes, image_shapes, targets=None, query_img_as_gallery=False):
        ...
        if self.training:
            ...
            ious_1st = self.get_IoU(boxes, targets)  # 此处box和pid2对应
        else:
        ...
        if self.training:
            ...
            boxes = [boxes_per_image.detach() for boxes_per_image in boxes]
            ious_2nd_corr = self.get_IoU(boxes, targets)  # 此处box和2nd对应
            if self.use_diff_thresh:
                self.proposal_matcher = det_utils.Matcher(
            ...
            ious_2nd = self.get_IoU(boxes, targets)  # 与3rd对应
        else:
        ...
        # loss
        # box_embeddings_3rd = box_embeddings_3rd+box_embeddings_2nd
        ...
        # box_embeddings_4th = box_embeddings_4th+box_embeddings_3rd
        ...
        if self.training:
            ...
            losses['loss_rcnn_reg_1st'] = losses['loss_rcnn_reg_1st']
            losses['loss_rcnn_reg_2nd'] = losses['loss_rcnn_reg_2nd']
            losses['loss_rcnn_reg_3rd'] = losses['loss_rcnn_reg_3rd']
            # reid 和 iou统一阶段预测 是否使用其预测结果做为动量呢？我认为可以，但原论文使用rpn的bbox来进行筛
            # 待测试
            # ious_1st = torch.clamp(ious_1st, min=self.bg_iou_thresh_1st)
            # ious_2nd = torch.clamp(ious_2nd, min=self.bg_iou_thresh_2nd)
            # ious_3rd = torch.clamp(ious_3rd, min=self.bg_iou_thresh_3rd)
            # print(box_pid_labels_2nd)
            # print(box_pid_labels_3rd)
            # print(box_pid_labels_4th)
            # print('---------------------------')
            # test 一下
            temp = 0.99
            loss_rcnn_reid_2nd, _, feats_reid_2nd, targets_reid_2nd = self.reid_loss_2nd(
                box_embeddings_2nd, box_pid_labels_2nd, ious_1st)

            lut = self.reid_loss_2nd.lut
            self.reid_loss_3rd.ema(lut,temp)
            # 预测的box_1st得到的特征embedding2和reg2 因此iou 1st和embedding2同reg2 公用一个 pid2
            # 我需要一个和pid3对应的iou
            loss_rcnn_reid_3rd,_, feats_reid_3rd, targets_reid_3rd = self.reid_loss_3rd(
                box_embeddings_3rd, box_pid_labels_3rd, ious_2nd)

            lut = self.reid_loss_3rd.lut
            self.reid_loss_4th.ema(lut,temp)

            loss_rcnn_reid_4th, _, feats_reid_4th, targets_reid_4th, = self.reid_loss_4th(
                box_embeddings_4th, box_pid_labels_4th, ious_3rd)
            # 可添加   cls1 和 iou1st
            corr_2nd = self.corr_2nd(box_embeddings_2nd, box_cls_scores_2nd,box_pid_labels_2nd)
            # embeding2nd 和 reg2nd对应和pid2nd对应 需要在get_box后直接获得相关iou
            corr_3rd = self.corr_3rd(box_embeddings_3rd, box_cls_scores_3rd,box_pid_labels_3rd)
    ...
    def get_boxes(self, box_regression, proposals, image_shapes):
        ...
        # if (pred_boxes[:, 2:] <= pred_boxes[:, :2]).all():
        #     print(pred_boxes)
        all_boxes = []
