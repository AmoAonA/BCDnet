...
from torchvision.ops import MultiScaleRoIAlign, focal_loss
...
class COAT(nn.Module):
    ...
    def forward(self, images, targets=None, query_img_as_gallery=False):
        ...
        (_, rcnn_losses, feats_reid_2nd,
         targets_reid_2nd, feats_reid_3rd, targets_reid_3rd) = (
    ...
    def forward(self, features, boxes, image_shapes, targets=None, query_img_as_gallery=False):
       ...
       # box_embeddings_4th = self.fuse_embedding_34(box_embeddings_3rd, box_embeddings_4th)
        ...
        return result, losses, feats_reid_2nd, targets_reid_2nd, feats_reid_3rd, targets_reid_3rd
            #,feats_reid_4th,targets_reid_4th
...
class information_flow(nn.Module):
    def __init__(self, in_chans):
        ...
        self.fc0 = nn.Linear(in_chans, in_chans)
        self.fc1 = nn.Sequential(nn.Linear(in_chans * 2, in_chans), norm_layer(in_chans))
        init.normal_(self.fc1[0].weight, std=0.01)
        init.constant_(self.fc1[0].bias, 0)
        self.act = nn.GELU()

    def forward(self, pred_feat, feat):
        pred_feat = self.fc0(pred_feat)
        feat = torch.cat([pred_feat, feat],dim=1)
        feat = self.fc1(feat)
        return feat
...
def detection_losses(
    ...
    print('box_regs_1st',box_regs_1st)
    print('box_target_regs_1st',box_reg_targets_1st)
    # --------------------- The second stage -------------------- #
...
if __name__ == "__main__":
    ...
    cfg.merge_from_file('..//configs//prw.yaml')

    model = COAT(cfg)
    images_ = [torch.rand((3, 1080, 1920)), torch.rand((3, 1080, 1920))]
    targets_ = [{'img_name': 'c5s1_056823.jpg',
                 'boxes': torch.tensor([[894., 318., 954., 514.]]),
                 'labels': torch.tensor([228])},
                {'img_name': 'c3s1_042901.jpg',
                 'boxes': torch.tensor([[1633., 307., 1824., 636.],
                                        [1535., 304., 1622., 597.],
                                        [1240., 422., 1277., 542.],
                                        [1203., 427., 1240., 543.]]),
                 'labels': torch.tensor([150, 155, 177, 178])}]
    model(images_, targets_)